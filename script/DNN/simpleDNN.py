...
def main() -> None:
    ...
    x_test = embedd_layer(x)  # 测试数据不加噪音
    y_train = torch.arange(4).repeat(100)

    # 将数据封装成Dataset 和 dataloder
    train_dataset = TensorDataset(x_train, y_train)
    test_dataset = TensorDataset(x_test, y_train)

    train_dataloader = DataLoader(train_dataset,  # 封装的对象
                                  batch_size=40,  # 输出的batch size
                                  shuffle=True,  # 随机输出
                                  num_workers=0)  # 只有1个进程
    test_dataloader = DataLoader(test_dataset,  # 封装的对象
                                 batch_size=40,  # 输出的batch size
                                 shuffle=True,  # 随机输出
                                 num_workers=0)  # 只有1个进程
    ...
    train_obj = TrainNet(model_x, train_dataloader,
                         test_dataloader, loss, updater)
